...
-- 3 --
select  c.cus_gender as Customer_Gender, count(*) as Order_Count, sum(o.ord_amount) as Order_amount 
from customer as c inner join `order` as o on o.cus_id = c.cus_id 
where ord_amount >= 3000 group by c.cus_gender;
 
 SELECT ORDERS, PRODUCT.PRO_NAME
FROM PRODUCT, SUPPLIER_PRICING, ORDERS
WHERE ORDERS.PRICING_ID = SUPPLIER_PRICING.PRICING_ID AND SUPPLIER_PRICING.PRO_ID = PRODUCT.PRO_ID 
AND ORDERS.CUS_ID = 2;
 -- 5 --
